...
exports.exploreCategoriesById = async (req, res, next) => {
	try {
		...
		const limitNumber = 20;
		const catgoryById = await Recipe.find({ 'category': catgoryId }).limit(limitNumber);

		res.render("categories", { title: "Cooking Blog - Categories", catgoryById });
	} catch (error) {
		res.status(500).send({ message: error.message || 'Something went wrong' })
	}
}
...
exports.submitRecipeOnPost = async (req, res, next) => {
	...
	try {
		...
		if(!req.files || Object.keys(req.files).length === 0) {
			console.log('No File were uploded.');
		} else {
